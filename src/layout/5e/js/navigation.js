...
//     this._addElement_li(null, "index.html", "Home", {
//       isRoot: true
//     });
...
//           const {ManageBrewUi} = await import("./utils-brew/utils-brew-ui-manage.js");
//           await ManageBrewUi.pOnClickBtnExportListAsUrl({
//             ele
//           });
//         }
//         ,
...
//     this._addElement_li(NavBar._CAT_UTILITIES, NavBar._getCurrentWikiHelpPage(), "Help", {
//       isExternal: true
//     });
//     this._addElement_divider(NavBar._CAT_UTILITIES);
...
//     }, );
//     this._addElement_dropdown(NavBar._CAT_SETTINGS, NavBar._CAT_CACHE, {
//       isSide: true
//     });
...
//     const [adventureBookIndex] = await Promise.all([DataUtil.loadJSON(`${Renderer.get().baseUrl}data/generated/gendata-nav-adventure-book-index.json`), ExcludeUtil.pInitialise(), ]);
//     const [prerelease,brew] = await Promise.all([PrereleaseUtil.pGetBrewProcessed(), BrewUtil2.pGetBrewProcessed(), ]);
//     [{
//       prop: "book",
...
//         const metas = [...adventureBookIndex[prop], ...fromPrerelease, ...fromBrew].filter(it => !ExcludeUtil.isExcluded(UrlUtil.encodeForHash(it.id), prop, it.source, {
//           isNoCount: true
//         }));
//         if (!metas.length)
//           return;
...
//             this._addElement_label(parentCategory, displayName, {
...
//                     }, );
//                   }
//                   let cleanName = indexMeta.name.startsWith(indexMeta.parentName) ? indexMeta.name.slice(indexMeta.parentName.length).replace(/^:\s+/, "") : indexMeta.name;
...
//     a.href = href;
//     a.innerHTML = `${this._addElement_getDatePrefix({
//       date: opts.date,
//       isAddDateSpacer: opts.isAddDateSpacer
//     })}${this._addElement_getSourcePrefix({
//       source: opts.source
//     })}${aText}${this._addElement_getSourceSuffix({
//       source: opts.source
//     })}`;
//     a.classList.add("nav__link");
//     if (opts.isInAccordion)
//       a.classList.add(`nav2-accord__lnk-item`, `inline-block`, `w-100`);
//     if (opts.isExternal) {
...
//     });
//   }
//   static _addElement_accordion(parentCategory, category, {date=null, isAddDateSpacer=false, source=null, }={}, ) {
//     const parentNode = this._getNode(parentCategory);
//     const li = document.createElement("li");
...
//     dispText.innerHTML = `${this._addElement_getDatePrefix({
//       date,
//       isAddDateSpacer
//     })}${this._addElement_getSourcePrefix({
//       source
...
//     li.innerHTML = `${this._addElement_getDatePrefix({
//       date,
//       isAddDateSpacer
//     })}${html}`;
//     parentNode.body.appendChild(li);
...
//       NavBar._handleDropdownClick(this, event, isSide);
//     }
//     ;
//     if (isSide) {
...
//       eleSpan.className = options.className;
//     eleSpan.onclick = options.click;
//     eleSpan.innerHTML = options.html;
//     if (options.context)
//       eleSpan.oncontextmenu = options.context;
//     if (options.title)
...
//         const eleSpan = document.createElement("span");
//         eleSpan.className = [className, "inline-block", i ? null : "w-100 min-w-0", ].filter(Boolean).join(" ");
//         eleSpan.onclick = click;
//         eleSpan.innerHTML = html;
//         if (title)
//           eleSpan.setAttribute("title", title);
//         li.appendChild(eleSpan);
//       }
//     );
//     parentNode.body.appendChild(li);
...
//     let hash = "";
//     if (typeof _SEO_PAGE !== "undefined")
//       currentPage = `${_SEO_PAGE}.html`;
...
//           node.isActive = false;
//           if (node.children)
//             _doRemoveAllPageHighlightsInner(node.children);
//         }
//       }
//     ;
...
//     this._openDropdown_mutAlignment({
//       liNavbar: lisOpen.slice(-1)[0]
//     });
//   }
//   static _openDropdown_mutAlignment({liNavbar}) {
//     const uls = [...liNavbar.querySelectorAll("ul.ve-dropdown-menu")];
//     const widthRequired = window.innerWidth < 1200 ? Math.max(...uls.map(ul => ul.getBoundingClientRect().width)) : uls.map(ul => ul.getBoundingClientRect().width).reduce( (a, b) => a + b, 0);
...
//     const $ele = $(ele);
//     const timerIds = $ele.siblings("[data-timer-id]").map( (i, e) => ({
//       $ele: $(e),
//       timerId: $(e).data("timer-id")
//     })).get();
...
//       JqueryUtil.doToast({
//         type: "danger",
//         content: `Failed to load state! ${VeCt.STR_SEE_CONSOLE}`
//       });
//       throw e;
//     }
//   }
...
//     if (this.parent)
//       this.parent.isActive = this._isActive;
//   }
//   get isActive() {
...
//   static getDispToggleDisplayHtml(val) {
//     return val ? `[\u2212]` : `[+]`;
//   }
